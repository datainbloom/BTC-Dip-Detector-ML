...
with tab_ml:
    ...
    st.markdown(
        """
        Forecast of a ≥20% Bitcoin dip in the next **30 days**, generated by a
        trained **Random Forest (scikit-learn)** model on recent market indicators.
        """
    )
    st.caption("⚡ Powered by Random Forest — Built with Scikit-Learn")
    ...
    try:
        ...
        with st.expander("Model details & methodology"):
            st.markdown(
                ...
                **Target event:** A drawdown of **≥20%** within the next **30 days**.  
                ...
                **How to interpret the probability:**
                - **0–39%** → Low risk (🟢): No alert by default  
                - **40–69%** → Medium risk (🟠): Keep an eye on momentum & volume  
                - **70–100%** → High risk (🔴): Consider risk management actions  
                
                **Notes:**  
                - The model uses recent market indicators and historical BTC data to estimate the likelihood
                  of a ≥20% drawdown in a 30-day window.  
